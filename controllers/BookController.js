...
export const bookNow = async (req, res) => {
  try {
    ...
    const captureResponse = await axios.post(`${PAYPAL_API}/v2/checkout/orders/${orderResponse.data.id}/capture`, {}, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`
      }
    ...
    console.log('PayPal payment captured successfully:', captureResponse.data);
    ...
    return res.status(200).json({ 
      booking, 
      payment: captureResponse.data, // Include the payment capture response
